/**
 * ✅ LEXLABOR V5 FINAL - 29 OCTUBRE 2025
 * Sistema de Gestión Jurídica Laboral
 * Versión: 5.0.20241029
 * 
 * IMPORTANTE: Si ves este mensaje en la consola, 
 * significa que estás ejecutando la VERSIÓN 5 FINAL.
 */

console.log('%c✅ LEXLABOR V5 FINAL - 29 OCT 2025', 'background: #ef4444; color: white; padding: 10px 20px; font-size: 16px; font-weight: bold; border-radius: 5px;');
console.log('%cVersión: 5.0.20241029', 'color: #22c55e; font-size: 14px; font-weight: bold;');
console.log('%cSi ves versiones anteriores, limpia el caché del navegador (Ctrl+Shift+R o Cmd+Shift+R)', 'color: #f59e0b; font-size: 12px;');

// Mock Data Storage - Casos Kanban
const casosKanban = {
  demanda_inicial: [
    { expediente: 'JLC/001/2023', actor: 'Juan Pérez López', demandado: 'Constructora Mexicana S.A.', juzgado: 'Juzgado 1º Laboral', urgente: false, fecha_actualizacion: '2024-10-15' },
    { expediente: 'JLC/005/2023', actor: 'María López García', demandado: 'Textiles del Norte S.A.', juzgado: 'Juzgado 2º Laboral', urgente: true, fecha_actualizacion: '2024-10-20' },
    { expediente: 'JLC/008/2023', actor: 'Carlos Gómez Ruiz', demandado: 'Servicios XYZ S.A.', juzgado: 'Juzgado 3º Laboral', urgente: false, fecha_actualizacion: '2024-10-18' }
  ],
  conciliacion: [
    { expediente: 'JLC/002/2023', actor: 'Ana Martínez Cruz', demandado: 'Manufacturas ABC S.A.', juzgado: 'Juzgado 1º Laboral', urgente: false, fecha_actualizacion: '2024-10-12' },
    { expediente: 'JLC/006/2023', actor: 'Luis Hernández Mora', demandado: 'Corporativo DEF S.A.', juzgado: 'Juzgado 4º Laboral', urgente: true, fecha_actualizacion: '2024-10-22' }
  ],
  demanda_excepciones: [
    { expediente: 'JLC/003/2023', actor: 'Pedro García Santos', demandado: 'Construcciones GHI S.A.', juzgado: 'Juzgado 2º Laboral', urgente: false, fecha_actualizacion: '2024-10-10' },
    { expediente: 'JLC/007/2023', actor: 'Rosa Sánchez Pérez', demandado: 'Industrias JKL S.A.', juzgado: 'Juzgado 3º Laboral', urgente: false, fecha_actualizacion: '2024-10-16' },
    { expediente: 'JLC/010/2023', actor: 'Jorge Ramírez Torres', demandado: 'Servicios MNO S.A.', juzgado: 'Juzgado 5º Laboral', urgente: true, fecha_actualizacion: '2024-10-25' },
    { expediente: 'JLC/012/2023', actor: 'Elena Torres Díaz', demandado: 'Comercializadora PQR', juzgado: 'Juzgado 1º Laboral', urgente: false, fecha_actualizacion: '2024-10-08' }
  ],
  ofrecimiento_pruebas: [
    { expediente: 'JLC/004/2023', actor: 'Sofía Jiménez Vargas', demandado: 'Distribuidora STU S.A.', juzgado: 'Juzgado 4º Laboral', urgente: false, fecha_actualizacion: '2024-10-14' },
    { expediente: 'JLC/009/2023', actor: 'Ricardo Morales Luna', demandado: 'Logística VWX S.A.', juzgado: 'Juzgado 2º Laboral', urgente: false, fecha_actualizacion: '2024-10-19' },
    { expediente: 'JLC/011/2023', actor: 'Patricia Díaz Romero', demandado: 'Transportes YZ S.A.', juzgado: 'Juzgado 3º Laboral', urgente: true, fecha_actualizacion: '2024-10-21' }
  ],
  desahogo_pruebas: [
    { expediente: 'JLC/013/2023', actor: 'Miguel Ángel Reyes', demandado: 'Empresa AAA S.A.', juzgado: 'Juzgado 5º Laboral', urgente: false, fecha_actualizacion: '2024-10-11' },
    { expediente: 'JLC/014/2023', actor: 'Laura Fernández Castro', demandado: 'Compañía BBB S.A.', juzgado: 'Juzgado 1º Laboral', urgente: true, fecha_actualizacion: '2024-10-23' }
  ]
};

const etapasConfig = {
  demanda_inicial: { nombre: 'Demanda Inicial', color: '#3B82F6', icono: 'fas fa-file-alt' },
  conciliacion: { nombre: 'Conciliación', color: '#F59E0B', icono: 'fas fa-handshake' },
  demanda_excepciones: { nombre: 'Demanda y Excepciones', color: '#8B5CF6', icono: 'fas fa-balance-scale' },
  ofrecimiento_pruebas: { nombre: 'Ofrecimiento de Pruebas', color: '#10B981', icono: 'fas fa-clipboard-list' },
  desahogo_pruebas: { nombre: 'Desahogo de Pruebas', color: '#F97316', icono: 'fas fa-gavel' }
};
  ...
  switch(view) {
    case 'dashboard':
      ...
      container.innerHTML = `
        ...
        <div class="card mb-20">
          <div class="card-header">
            <h3 class="card-title"><i class="fas fa-project-diagram"></i> Etapas Procesales - Vista Kanban</h3>
            <select class="form-control" style="width: 200px;" id="kanbanFilterAbogado">
              <option value="todos">Todos mis casos</option>
              <option value="activos">Casos activos</option>
              <option value="ultimos6">Últimos 6 meses</option>
            </select>
          </div>
          <div class="card-body" style="padding: 0; overflow-x: auto;">
            <div style="display: flex; gap: 16px; padding: 20px; min-width: max-content;" id="kanbanBoardAbogado">
              ${Object.keys(etapasConfig).map(etapaKey => {
                const etapa = etapasConfig[etapaKey];
                const casos = casosKanban[etapaKey] || [];
                return `
                  <div style="min-width: 300px; background: var(--color-surface); border-radius: var(--radius-lg); border: 1px solid var(--color-card-border); padding: 16px;">
                    <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 16px; padding-bottom: 12px; border-bottom: 3px solid ${etapa.color};">
                      <div style="display: flex; align-items: center; gap: 8px;">
                        <i class="${etapa.icono}" style="color: ${etapa.color}; font-size: 18px;"></i>
                        <span style="font-weight: 600; font-size: 14px;">${etapa.nombre}</span>
                      </div>
                      <span style="background: rgba(${etapa.color === '#3B82F6' ? '59, 130, 246' : etapa.color === '#F59E0B' ? '245, 158, 11' : etapa.color === '#8B5CF6' ? '139, 92, 246' : etapa.color === '#10B981' ? '16, 185, 129' : '249, 115, 22'}, 0.15); padding: 4px 10px; border-radius: var(--radius-full); font-size: 12px; font-weight: 600; color: ${etapa.color};">${casos.length}</span>
                    </div>
                    <div class="kanban-column-items" data-etapa="${etapaKey}" style="display: flex; flex-direction: column; gap: 12px; min-height: 200px;">
                      ${casos.map(caso => `
                        <div class="kanban-case-card" draggable="true" data-expediente="${caso.expediente}" data-etapa="${etapaKey}" style="background: var(--color-surface); border-left: 4px solid ${etapa.color}; border-radius: 6px; padding: 12px; cursor: move; box-shadow: var(--shadow-sm); transition: all 0.2s;" onmouseover="this.style.boxShadow='var(--shadow-md)'; this.style.transform='translateY(-2px)';" onmouseout="this.style.boxShadow='var(--shadow-sm)'; this.style.transform='translateY(0)';" onclick="openKanbanCaseModal('${caso.expediente}', '${etapaKey}')">
                          <div style="display: flex; justify-content: space-between; align-items: start; margin-bottom: 8px;">
                            <div style="font-weight: 600; font-size: 13px; color: var(--color-text);">${caso.expediente}</div>
                            ${caso.urgente ? '<i class="fas fa-exclamation-circle" style="color: var(--color-error); font-size: 14px;" title="Urgente"></i>' : ''}
                          </div>
                          <div style="font-size: 12px; color: var(--color-text); margin-bottom: 4px; font-weight: 500;">${caso.actor}</div>
                          <div style="font-size: 11px; color: var(--color-text-secondary); margin-bottom: 8px;">vs</div>
                          <div style="font-size: 11px; color: var(--color-text-secondary); margin-bottom: 8px;">${caso.demandado}</div>
                          <div style="display: flex; align-items: center; gap: 4px; font-size: 10px; color: var(--color-text-secondary); margin-bottom: 4px;">
                            <i class="fas fa-university"></i>
                            <span>${caso.juzgado}</span>
                          </div>
                          <div style="font-size: 10px; color: var(--color-text-secondary);">
                            <i class="fas fa-clock"></i> ${new Date(caso.fecha_actualizacion).toLocaleDateString('es-MX', { day: 'numeric', month: 'short' })}
                          </div>
                        </div>
                      `).join('')}
                    </div>
    ...
    <div class="card">
      <div class="card-header" style="border-bottom: none; padding-bottom: 0;">
        <div style="display: flex; gap: 8px; flex-wrap: wrap;" id="casoTabs">
          ...
          <button class="btn btn--sm tab-btn" data-tab="kanban">Kanban</button>
...
function loadCasoTab(tabName, caso) {
  ...
  switch(tabName) {
    ...
    case 'kanban':
      // Find which stage this case is in
      let currentCaseStage = 'demanda_inicial';
      let currentCaseData = null;
      
      for (let etapaKey in casosKanban) {
        const found = casosKanban[etapaKey].find(c => c.expediente === caso.expediente);
        if (found) {
          currentCaseStage = etapaKey;
          currentCaseData = found;
          break;
        }
      }
      
      content.innerHTML = `
        <h3 class="mb-20">Vista Kanban - Etapa del Caso</h3>
        
        <div class="card mb-20" style="background: var(--color-bg-1);">
          <div class="card-body">
            <div style="display: flex; justify-content: between; align-items: center; gap: 16px;">
              <div style="flex: 1;">
                <h4 style="margin-bottom: 8px;">Etapa Actual</h4>
                <div style="display: flex; align-items: center; gap: 8px;">
                  <i class="${etapasConfig[currentCaseStage].icono}" style="color: ${etapasConfig[currentCaseStage].color}; font-size: 20px;"></i>
                  <span style="font-size: 18px; font-weight: bold; color: ${etapasConfig[currentCaseStage].color};">${etapasConfig[currentCaseStage].nombre}</span>
                </div>
              </div>
              <div>
                <label class="form-label" style="margin-bottom: 8px;">Cambiar Etapa:</label>
                <select class="form-control" id="cambiarEtapaSelect" onchange="cambiarEtapaCaso('${caso.expediente}', this.value)">
                  ${Object.keys(etapasConfig).map(key => `
                    <option value="${key}" ${key === currentCaseStage ? 'selected' : ''}>${etapasConfig[key].nombre}</option>
                  `).join('')}
                </select>
              </div>
            </div>
        ...
        <div class="card mb-20">
          <div class="card-header">
            <h3 class="card-title">Vista de Etapas Adyacentes</h3>
          </div>
          <div class="card-body" style="overflow-x: auto;">
            <div style="display: flex; gap: 16px; min-width: max-content;">
              ${(() => {
                const allStages = Object.keys(etapasConfig);
                const currentIndex = allStages.indexOf(currentCaseStage);
                const stagesToShow = [];
                
                // Show previous stage if exists
                if (currentIndex > 0) stagesToShow.push(allStages[currentIndex - 1]);
                // Show current stage
                stagesToShow.push(currentCaseStage);
                // Show next stage if exists
                if (currentIndex < allStages.length - 1) stagesToShow.push(allStages[currentIndex + 1]);
                
                return stagesToShow.map(stageKey => {
                  const etapa = etapasConfig[stageKey];
                  const isCurrent = stageKey === currentCaseStage;
                  return `
                    <div style="min-width: 280px; background: ${isCurrent ? 'var(--color-bg-1)' : 'var(--color-surface)'}; border-radius: var(--radius-lg); border: ${isCurrent ? '2px solid ' + etapa.color : '1px solid var(--color-card-border)'}; padding: 16px;">
                      <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 12px; padding-bottom: 8px; border-bottom: 2px solid ${etapa.color};">
                        <i class="${etapa.icono}" style="color: ${etapa.color};"></i>
                        <span style="font-weight: 600; font-size: 13px;">${etapa.nombre}</span>
                        ${isCurrent ? '<span style="background: ' + etapa.color + '; color: white; padding: 2px 8px; border-radius: 4px; font-size: 10px; margin-left: auto;">ACTUAL</span>' : ''}
                      </div>
                      ${isCurrent ? `
                        <div style="background: var(--color-surface); border-left: 4px solid ${etapa.color}; border-radius: 6px; padding: 12px; box-shadow: var(--shadow-md);">
                          <div style="font-weight: 600; font-size: 13px; margin-bottom: 6px;">${caso.expediente}</div>
                          <div style="font-size: 12px; color: var(--color-text); margin-bottom: 4px;">${caso.actor}</div>
                          <div style="font-size: 11px; color: var(--color-text-secondary);">vs ${caso.demandado}</div>
                        </div>
                      ` : '<div style="text-align: center; padding: 20px; color: var(--color-text-secondary); font-size: 12px;">Etapa ' + (stageKey < currentCaseStage ? 'anterior' : 'siguiente') + '</div>'}
                    </div>
                  `;
                }).join('');
              })()}
        ...
        <div class="card">
          <div class="card-header">
            <h3 class="card-title">Historial de Cambios de Etapa</h3>
          </div>
          <div class="card-body">
            <div style="display: flex; flex-direction: column; gap: 12px;">
              <div style="padding: 12px; background: var(--color-bg-1); border-left: 3px solid #3B82F6; border-radius: var(--radius-base);">
                <div style="display: flex; justify-content: space-between; margin-bottom: 4px;">
                  <strong style="font-size: 13px;">Demanda Inicial</strong>
                  <span style="font-size: 11px; color: var(--color-text-secondary);">15 Ene 2023</span>
                </div>
                <div style="font-size: 12px; color: var(--color-text-secondary);">Caso creado en esta etapa</div>
              </div>
              <div style="padding: 12px; background: var(--color-bg-2); border-left: 3px solid #F59E0B; border-radius: var(--radius-base);">
                <div style="display: flex; justify-content: space-between; margin-bottom: 4px;">
                  <strong style="font-size: 13px;">Conciliación</strong>
                  <span style="font-size: 11px; color: var(--color-text-secondary);">20 Mar 2023</span>
                </div>
                <div style="font-size: 12px; color: var(--color-text-secondary);">Movido por ${caso.abogado}</div>
              </div>
              <div style="padding: 12px; background: var(--color-bg-5); border-left: 3px solid #8B5CF6; border-radius: var(--radius-base);">
                <div style="display: flex; justify-content: space-between; margin-bottom: 4px;">
                  <strong style="font-size: 13px;">Demanda y Excepciones</strong>
                  <span style="font-size: 11px; color: var(--color-text-secondary);">15 Jun 2023</span>
                </div>
                <div style="font-size: 12px; color: var(--color-text-secondary);">Movido por ${caso.abogado}</div>
              </div>
            </div>
...
// Helper Functions
// Kanban Drag & Drop Initialization
  ...
  setTimeout(() => {
    ...
    columns.forEach(column => {
      new Sortable(column, {
        ...
        handle: '.kanban-case-card',
        onEnd: function(evt) {
          const expediente = evt.item.dataset.expediente;
          const oldEtapa = evt.item.dataset.etapa;
          const newEtapa = evt.to.dataset.etapa;
          
          if (oldEtapa !== newEtapa) {
            // Find and move the case
            const casoIndex = casosKanban[oldEtapa].findIndex(c => c.expediente === expediente);
            if (casoIndex !== -1) {
              const caso = casosKanban[oldEtapa].splice(casoIndex, 1)[0];
              casosKanban[newEtapa].push(caso);
              
              // Update the card's data attribute
              evt.item.dataset.etapa = newEtapa;
              
              // Update counter badges
              updateKanbanCounters();
              
              showToast(`Caso ${expediente} movido a ${etapasConfig[newEtapa].nombre}`, 'success');
            }
          }
        }
      });
...
function updateKanbanCounters() {
  Object.keys(etapasConfig).forEach(etapaKey => {
    const count = casosKanban[etapaKey]?.length || 0;
    const counter = document.querySelector(`[data-etapa="${etapaKey}"]`)?.parentElement?.querySelector('span[style*="border-radius: var(--radius-full)"]');
    if (counter) {
...
function openKanbanCaseModal(expediente, etapa) {
  const caso = casosKanban[etapa].find(c => c.expediente === expediente);
  if (!caso) return;
  ...
  const modalHTML = `
    <div class="modal-overlay" onclick="closeModal(event)">
      <div class="modal" onclick="event.stopPropagation()">
        <div class="modal-header">
          <h3 class="modal-title">Resumen Rápido - ${caso.expediente}</h3>
          <button class="btn-icon" onclick="closeModal()">
            <i class="fas fa-times"></i>
          </button>
        </div>
        <div class="modal-body">
          <div style="display: flex; flex-direction: column; gap: 12px;">
            <div>
              <strong style="font-size: 12px; color: var(--color-text-secondary); display: block; margin-bottom: 4px;">Expediente:</strong>
              <span style="font-size: 16px; font-weight: 600;">${caso.expediente}</span>
              ${caso.urgente ? '<span class="status-badge pendiente" style="margin-left: 8px;"><i class="fas fa-exclamation-circle"></i> Urgente</span>' : ''}
            </div>
            <div>
              <strong style="font-size: 12px; color: var(--color-text-secondary); display: block; margin-bottom: 4px;">Actor:</strong>
              <span style="font-size: 14px;">${caso.actor}</span>
            </div>
            <div>
              <strong style="font-size: 12px; color: var(--color-text-secondary); display: block; margin-bottom: 4px;">Demandado:</strong>
              <span style="font-size: 14px;">${caso.demandado}</span>
            </div>
            <div>
              <strong style="font-size: 12px; color: var(--color-text-secondary); display: block; margin-bottom: 4px;">Juzgado:</strong>
              <span style="font-size: 14px;">${caso.juzgado}</span>
            </div>
            <div>
              <strong style="font-size: 12px; color: var(--color-text-secondary); display: block; margin-bottom: 4px;">Etapa Actual:</strong>
              <span style="font-size: 14px; color: ${etapasConfig[etapa].color}; font-weight: 600;">
                <i class="${etapasConfig[etapa].icono}"></i> ${etapasConfig[etapa].nombre}
              </span>
            </div>
            <div>
              <strong style="font-size: 12px; color: var(--color-text-secondary); display: block; margin-bottom: 4px;">Última Actualización:</strong>
              <span style="font-size: 14px;">${new Date(caso.fecha_actualizacion).toLocaleDateString('es-MX', { year: 'numeric', month: 'long', day: 'numeric' })}</span>
            </div>
          </div>
        </div>
        <div class="modal-footer">
          <button class="btn btn--secondary" onclick="closeModal()">Cerrar</button>
          <button class="btn btn--primary" onclick="closeModal(); navigateToCasoDetails('${caso.expediente}');">
...
function cambiarEtapaCaso(expediente, newEtapa) {
  // Find current stage
  let currentEtapa = null;
  for (let etapaKey in casosKanban) {
    if (casosKanban[etapaKey].find(c => c.expediente === expediente)) {
      currentEtapa = etapaKey;
      break;
    }
  }
  
  if (currentEtapa && currentEtapa !== newEtapa) {
    const casoIndex = casosKanban[currentEtapa].findIndex(c => c.expediente === expediente);
    if (casoIndex !== -1) {
      const caso = casosKanban[currentEtapa].splice(casoIndex, 1)[0];
      casosKanban[newEtapa].push(caso);
      
      showToast(`Caso ${expediente} movido a ${etapasConfig[newEtapa].nombre}`, 'success');
      
      // Reload the tab
      const casoData = appData.casos.find(c => c.expediente === expediente) || appData.casos[0];
      loadCasoTab('kanban', casoData);
    }
  }
}
